...
// node_modules/@yoimu/common-lib/dist/index.esm.js
var createEnum = (obj) => {
  const translation = {};
  const reverseEnum = {};
  const $enum = {};
  const keys = [];
  const t2 = (val) => translation[val];
  const key = (val) => reverseEnum[val];
  const map = (callback) => callback ? keys.map((k, i) => callback($enum[k], k, i)) : keys;
  ...
  return { ...$enum, t: t2, key, keys, map, reduce };
};
var declareEnum = (obj, typeBindValue) => {
  const keys = [];
  let length = 0;
  const map = (callback) => {
    if (!callback)
...
var toTraditional = (text) => {
  return t(text, "s");
};
var checkSameChinese = (text1, text2) => {
...
var checkIncludeText = (text, keyword) => {
...
var today = () => [moment().startOf("day"), moment().endOf("day")];
var yesterday = () => [
...
var thisMonth = () => [moment().date(1).startOf("day"), moment().endOf("day")];
var pastMonth = () => [
  moment().month(moment().month() - 1).date(1).startOf("day"),
...
var start$2 = (state) => (callback, delay = 0) => {
  stop$2(state)();
  state.timer = setTimeout(() => callback(), delay);
};
var startSync = (state) => (promiseFun, delay = 0) => {
...
var start$1 = (state) => (callback, delay = 0) => {
  stop$1(state)();
...
var mergeWords = (...conditionTextArr) => {
  if (!conditionTextArr)
    return "";
  let resultWord = "";
  let first = true;
...
@yoimu/common-lib/dist/index.esm.js:
  (*! moment.js *)
*/
//# sourceMappingURL=@yoimu_common-lib.js.map
